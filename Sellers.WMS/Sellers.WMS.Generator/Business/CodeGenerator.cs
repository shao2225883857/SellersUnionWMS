...
namespace Sellers.WMS.Generator.Business
{
    ...
    public partial class CodeGenerator
    {
        ...
        private void GetCodeUsing(bool table)
        {
            ...
            this.CodeText.AppendLine("using Sellers.WMS.Utils.AbstractModel;");
            if (!table)
            {
                this.CodeText.AppendLine("using FluentNHibernate.Mapping;");
        ...
        private void GetCodeNamespace(string space, bool table)
        {
            ...
            this.CodeText.Append(this.project + "." + space);
            this.CodeText.AppendLine(string.Empty);
            this.CodeText.AppendLine("{");
        ...
        private void GetCodeClassName()
        {
            ...
            {
                case "TypeMap":
                    this.CodeText.AppendLine("    public class " + this.className + "TypeMap : BaseEntityMap<" + this.className + "Type> ");
                    break;
                case "Domian":
                    this.CodeText.AppendLine("    public class " + this.className + "Type : BaseEntity");
                    break;
                case "Manager":
            ...
            for (int i = 0; i < xmlNode.ChildNodes.Count; i++)
            {
                if (((XmlNode)xmlNode.ChildNodes[i]).LocalName.Equals("Columns"))
                {
                    for (int j = 0; j < xmlNode.ChildNodes[i].ChildNodes.Count; j++)
                    {
                        ...
                        this.CodeText.AppendLine("        /// </summary>");
                        this.CodeText.AppendLine("        public virtual " + dataType + " " + field + " { get; set; }");
